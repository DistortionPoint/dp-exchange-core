...
#### Phase 8 · Instruments and search (11) — **complete**
...
- [x] `gemini   ` GET    /v1/feepromos
- [x] `gemini   ` GET    /v1/symbols
- [x] `gemini   ` ~~GET~~ POST /v1/wrap/:symbol — enumeration error: the spec has only a POST, done as `convert/4` in Phase 3
- [x] `gemini   ` GET    /v2/network/USDC
- [x] `gemini   ` GET    /v2/networks/{network}/assets — the box said GET /v2/networks/; the spec has no such path
...
| 2026-09-01 | **v4.0 — Phases 6, 7 and 8's instruments group closed, in plan order after two rounds of skipping.** Phase 6 had been passed over entirely — its *Streaming* group was two boxes and done, while **Schwab's Streamer (17) and Gemini's WebSocket (22) were untouched** and work had moved to Phase 7. Both are now complete including transport: `websockex` added to `dp_exchange_schwab`, whose `mix.exs` said to add it "when it is implemented, and not before". **Schwab's field numbers disagree between services more than assumed** — field 1 is the bid in `LEVELONE_EQUITIES` and the description in `LEVELONE_OPTIONS`; field 6 is `ask_id`, `bid_id` and `total_volume` across equities, futures and forex; and **futures swaps `bid_id`/`ask_id` relative to equities**, so a shared map would report the bid's exchange as the ask's with both values real. **Gemini's socket contradicts its own REST API**: `/v1/trades` reports the taker's side, `@trade` reports `m` = "whether the buyer is the maker" — the opposite. Its timestamps are nanoseconds and its `depth` channels are differential, where a `U..u` gap check is the only defence against a silently wrong book. Phase 7 is complete: **five public/private path corrections on Coinbase** (book, candles, product, product list, and the sized top-of-book that had been reading a ticker with no sizes), Webull's equity market data behind a category switch, and Gemini's perpetual candles, which serve `1m` only and were being answered by the spot endpoint without error. Phase 8's instruments group is complete; two of its boxes were **enumeration errors** — `/v1/wrap/:symbol` is a POST and was already done as `convert/4`, and `GET /v2/networks/` is not a path the spec has. The contract is now 77 callbacks: `get_trades/2`, `get_auction_imbalance/2`, `get_volume_profile/3`, `get_fx_rate/3`, `list_networks/2`, `list_fee_promos/1`, plus `Types.Trade.broken`, `Types.AuctionImbalance`, `Types.VolumeProfile` and `Types.FxRate`. **`list_networks/2` is the one that matters most**: `get_deposit_address/3` takes a network and nothing said which a venue accepts, and a wrong one sends funds to a chain the venue does not credit. |
...
**Last Updated**: 2026-09-01 (v4.0 — **Implementing.** Phases 6 and 7 complete; Phase 8 in progress. Earlier: v3.9 — v3.8 — **Implementing.** Phase 3 complete, 30 of 30; the Venue contract is 71 callbacks. Earlier: v3.0 — **Implementing.** Phase 1 corrected Schwab's false streaming claims, migrated Webull's five undocumented paths and Gemini's transfers, added deprecation guards — and found Robinhood using an ask as a trade price, asserted as correct by its own tests.)
